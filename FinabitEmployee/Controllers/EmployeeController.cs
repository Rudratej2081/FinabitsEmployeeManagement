...
namespace FinabitEmployee.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public EmployeeController(UserManager<ApplicationUser> userManager, RoleManager<IdentityRole> roleManager, IConfiguration configuration, AppDbContext appDb, IHubContext<ChatHub> hubContext)
        {
      ...
      [FromQuery] int pageSize = 10)
        {
            ...
            // Building the query to filter salary slips
            var salarySlipsQuery = _context.salarySlips.AsQueryable();

            // If email is provided, filter by email, otherwise use logged-in user's email if no email is provided
            if (!string.IsNullOrEmpty(email))
            {
